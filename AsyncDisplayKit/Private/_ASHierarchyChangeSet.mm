...
- (void)_validateUpdate
{
  ...
  if (newSectionCount != oldSectionCount + insertedSectionCount - deletedSectionCount) {
    ASFailUpdateValidation(@"Invalid number of sections. The number of sections after the update (%zd) must be equal to the number of sections before the update (%zd) plus or minus the number of sections inserted or deleted (%tu inserted, %tu deleted)", (long)newSectionCount, (long)oldSectionCount, (long)insertedSectionCount, (long)deletedSectionCount);
    return;
  }
  ...
  if (invalidSectionDelete != NSNotFound) {
    ASFailUpdateValidation(@"Attempt to delete section %zd but there are only %zd sections before the update.", (long)invalidSectionDelete, (long)oldSectionCount);
    return;
  }
  ...
  for (_ASHierarchyItemChange *change in _deleteItemChanges) {
    for (NSIndexPath *indexPath in change.indexPaths) {
      ...
      if (section >= oldSectionCount) {
        ASFailUpdateValidation(@"Attempt to delete item %zd from section %zd, but there are only %zd sections before the update.", (long)item, (long)section, (long)oldSectionCount);
        return;
      }
      ...
      if (item >= oldItemCount) {
        ASFailUpdateValidation(@"Attempt to delete item %zd from section %zd, which only contains %zd items before the update.", (long)item, (long)section, (long)oldItemCount);
        return;
      }
    }
  }
  ...
  for (_ASHierarchyItemChange *change in _insertItemChanges) {
    for (NSIndexPath *indexPath in change.indexPaths) {
      ...
      if (section >= newSectionCount) {
        ASFailUpdateValidation(@"Attempt to insert item %zd into section %zd, but there are only %zd sections after the update.", (long)item, (long)section, (long)newSectionCount);
        return;
      }
      ...
      if (item >= newItemCount) {
        ASFailUpdateValidation(@"Attempt to insert item %zd into section %zd, which only contains %zd items after the update.", (long)item, (long)section, (long)newItemCount);
        return;
      }
    }
  }
  ...
  if (invalidSectionInsert != NSNotFound) {
    ASFailUpdateValidation(@"Attempt to insert section %zd but there are only %zd sections after the update.", (long)invalidSectionInsert, (long)newSectionCount);
    return;
  }
